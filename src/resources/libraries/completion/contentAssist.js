...
function contentAssistProvider( mVisitor, mTypes, proposalUtils, scriptedLogger ) {
	...
	function calculateFunctionProposal(name, typeObj, offset) {
		...
		for (var p = 0; p < plen; p++) {
			...
			if (optional) {
				// CGJ: use <> to avoid confusion with array results
				argName = '<' + argName + '>';
//				argName = '[' + argName + ']';
			}
	...
	function addLintGlobals(env, lintOptions) {
		...
		if (lintOptions && isArray(lintOptions.global)) {
			for (let i = 0; i < lintOptions.global.length; i++) {
		...
		computeProposals: function(buffer, offset, context) {
			...
			try {
				...
				if (completionKind) {
					...
					// add common lib
//					environment.mergeSummary( libCommon, environment.globalTypeName );
