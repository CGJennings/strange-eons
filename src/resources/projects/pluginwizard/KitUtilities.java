...
public class KitUtilities {
    ...
    /**
     * Creates one or more subfolders in a task folder. If path is {@code null},
     * then this will create the plug-in's base location, the location
     * identified by the "Location" field of {@link NamePage}. Typically, this
     * is something like resources/myplugin. If path is non-null, then it will
     * create a subfolder (using path as the name) within the base location. If
     * path contains one or more slashes (/), then the entire path will be
     * created. For example, "fonts/body" would create a fonts folder in the
     * base location, and a body folder within the fonts folder.
     *
     * <p>
     * If the name page np is {@code null}, then the task folder is used as the
     * base location.
     *
     * <p>
     * If a folder that would have been created by this method already exists,
     * the creation of that folder is skipped without causing an exception.
     *
     * @param t the task that is being created
     * @param np a name page that allowed the user to describe basic data about
     * the plug-in task
    ...
    public static void makeTaskFolders(Task t, NamePage np, String path) throws IOException {
        ...
        if (np != null) {
            parent = np.getPathAsFile(parent);
        }
    ...
    /**
     * Returns the text of a template file stored in
     * resources/projects/pluginwizard.
     *
     ...
     * @param type the plug-in type that will determine the values of the
     * conditionals
     */
    ...
    /**
     * Sets the file name used for a task's plug-in bundle when the Make Bundle
     * action is applied to it. This method uses the file name supplied by the
     * user on a {@link NamePage}.
     *
     * @param t the task whose bundle file should be set
    ...
    /**
     * Returns a File object for a file within a plug-in within the given task
     * folder. If the name page is non-{@code null}, then the base location
     * given on that page will be the parent of the file. Otherwise, the task
     * folder will be the parent of the file. The path argument specifies a path
     * relative to this parent and ending in the file of interest.
     *
     * @param t the task folder that contains the plug-in files
     * @param np if non-{@code null}, used to determine the base location within
     * the task
     * @param path a path indicating the desired file location, using / to
     * separate folders
