...
class VirtualDeckDialog extends javax.swing.JDialog {
    ...
    private DefaultListModel<Card> listToModel(JList<Card> li) {
        if (li == deckList) {
            return deck;
        }
        if (li == playList) {
    ...
    private void resetModelToDeck(DefaultListModel<Card> m) {
        while (m.getSize() > 0) {
            Card c = m.remove(m.getSize() - 1);
            c.owner = NOBODY;
            c.faceUp = false;
            c.drawDivider = false;
            deck.addElement(c);
    ...
    private void listValueChangedImpl(Object list) {
        @SuppressWarnings("unchecked")
        JList<Card> li = (JList<Card>) list;
        showCard(li.getSelectedValue());
    }
    ...
    private int getListIndex(JList<Card> list, MouseEvent evt) {
    ...
    private void redrawIndex(JList<Card> list, int i) {
    ...
    @SuppressWarnings("unchecked")
    private ListCellRenderer<Card> renderer = (ListCellRenderer<Card>) (ListCellRenderer<?>) new DefaultListCellRenderer() {
        @Override
