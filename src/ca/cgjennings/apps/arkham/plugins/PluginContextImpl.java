...
final class PluginContextImpl implements PluginContext {
     ...
     * plug-in can be {@code null} if a dummy context is required; this is used
     * when scripts are run during development.
     *
     * @param installedPlugin the installed plug-in that holds the plug-in
     * instance that the context applied to
     * @param plugin the plug-in to create the context for; if {@code null},
     * will be requested from the installedPlugin
     * @param modifiers if the plug-in is being activated, the mask for the
     * modifier keys that are held down; otherwise 0
     * @param isProbe {@code true} to set the context's information probe flag,
     * which indicates to the plug-in that only basic information will be
     * collected
